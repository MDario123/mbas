...
bin/mbas: tmp/mbas.o tmp/tomlc17.o
	mkdir -p bin
	cc tmp/mbas.o tmp/tomlc17.o -o bin/mbas $$(pkg-config --cflags --libs libpipewire-0.3)
	chmod +x bin/mbas

tmp/mbas.o: src/main.c src/*
	mkdir -p tmp
	cc -c src/main.c -o tmp/mbas.o $$(pkg-config --cflags --libs libpipewire-0.3)
...
bin/mbas_debug: tmp/mbas_debug.o tmp/tomlc17.o
	mkdir -p bin
	cc -g tmp/mbas_debug.o tmp/tomlc17.o -o bin/mbas_debug $$(pkg-config --cflags --libs libpipewire-0.3)
	chmod +x bin/mbas_debug

tmp/mbas_debug.o: src/main.c src/*
	echo "#define DEBUG \n" | cat - src/main.c > src/main_debug.c
	mkdir -p tmp
	cc -g -c src/main_debug.c -o tmp/mbas_debug.o $$(pkg-config --cflags --libs libpipewire-0.3)
	rm src/main_debug.c
